...
func TestRunMigrations(t *testing.T) {
	...
	err = db.QueryRowContext(t.Context(), "SELECT name FROM sqlite_master WHERE type='table' LIMIT 1").Scan(&tableName)
	require.NoError(t, err)
...
func TestRunMigrations_SessionsTable(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='sessions'").Scan(&count)
	require.NoError(t, err)
...
func TestRunMigrations_ParticipantsTable(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='participants'").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)
}
...
func TestRunMigrations_AudioStreamsTable(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='audio_streams'").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)
}
...
func TestRunMigrations_TranscriptionsTable(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='transcriptions'").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)
}
...
func TestRunMigrations_MinutesTable(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='minutes'").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)
}
...
func TestRunMigrations_WebhookEventsTable(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='webhook_events'").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)
}
...
func TestRunMigrations_ProcessingQueueTable(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='processing_queue'").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)
}
...
func TestRunMigrations_Indexes(t *testing.T) {
	...
	var indexCount int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND tbl_name='sessions'").Scan(&indexCount)
	require.NoError(t, err)
...
func TestRunMigrations_IndexesForParticipants(t *testing.T) {
	...
	var indexCount int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND tbl_name='participants'").Scan(&indexCount)
	require.NoError(t, err)
...
func TestRunMigrations_CreatesAllTables(t *testing.T) {
	...
	for _, tableName := range expectedTables {
		var count int
		err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name=?", tableName).Scan(&count)
		require.NoError(t, err)
...
func TestMigrationConstraints(t *testing.T) {
	...
	rows, err := db.QueryContext(t.Context(), "SELECT sql FROM sqlite_master WHERE type='table' AND name='sessions'")
	require.NoError(t, err)
	defer rows.Close()

	for rows.Next() {
		if scanErr := rows.Scan(&constraint); scanErr != nil {
			continue
		}
		if constraint != "" {
...
func TestMigrationIndexesOnStatus(t *testing.T) {
	...
	var indexCount int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND tbl_name='sessions' AND name LIKE 'idx_%'").Scan(&indexCount)
	require.NoError(t, err)
...
func TestMigrationIndexesOnDateColumns(t *testing.T) {
	...
	var count int
	err = db.QueryRowContext(t.Context(), "SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND name='idx_sessions_created'").Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)
}
...
func TestMigrationUniqueConstraints(t *testing.T) {
	...
	var constraint string
	rows, err := db.QueryContext(t.Context(), "SELECT sql FROM sqlite_master WHERE type='table' AND name='transcriptions'")
	require.NoError(t, err)
	defer rows.Close()
